...
parse_opt (int key, char *arg, struct argp_state *state)
{
  switch (key)
    ...
    case OPTION_CONSOLE_SOCKET:
      crun_context.console_socket = arg;
      break;
...
int
crunCmdCreate (struct crun_global_arguments *global_args, int argc, char **argv, libcrun_error_t *err)
{
  int first_arg = 0, ret;
  ...
  argp_parse (&run_argp, 4, argv, ARGP_IN_ORDER, &first_arg, &crun_context);
